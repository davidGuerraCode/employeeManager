        ...
        div(class="col s12 m8 offset-m2")
          div(class="login card-panel white grey-text text-darken-2 center")
            ...
            form
              ...
              button(class="btn teal white-text" @click="signIn") Sign In
              div
                p Not register? go and 
                  router-link(to="/signup" class="blue-text text-darken-3") Signup.
</template>
  ...
  methods: {
    signIn (event) {
      firebase.auth().signInWithEmailAndPassword(this.user.email, this.user.password)
        .then(user => {
          console.log(`Bienvenido de vuelta ${user.email}`)
